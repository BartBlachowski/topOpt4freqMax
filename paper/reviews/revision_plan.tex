...
\begin{tcolorbox}[colback=yellow!8,colframe=black,title={Comparator-evidence policy (July 2026)}]
The reconstruction campaign did not establish a paper-faithful implementation
of Du--Olhoff.  The \texttt{OlhoffApproachExact} and related reconstruction
paths are therefore archived diagnostics and are not reviewer evidence.  No
revision task, numerical table, convergence statement, frequency-gap claim,
speedup, or scaling claim may depend on those paths.  The only Olhoff-related
implementation permitted in production comparisons is
\texttt{analysis/OlhoffApproach}, which must be named the
\emph{local Olhoff-inspired comparison implementation}.  The published
Olhoff--Du algorithm may still be described as literature background, but it
must not be identified with the local implementation or used as its
quantitative validation target.
\end{tcolorbox}

\begin{tcolorbox}[colback=red!6,colframe=black,title={Performance-evidence policy (13 July 2026) --- supersedes every timing, speedup, and scaling item below}]
\textbf{EXP1} (local performance table: timing, memory, iteration counts,
$\omega_1$) and \textbf{EXP5} (log-log scaling fit, which consumed EXP1) are
\textbf{retired from the reviewer evidence chain}
(\texttt{examples/Revision\_v1/SCIENTIFIC\_DECISION\_EXP1\_EXP5.md}).  Gate~P1 and
planning items A2/A6 are \textbf{void}; no performance, timing, memory, or
scaling stage remains in the campaign, and comparator telemetry is deliberately
not implemented.  \textbf{No quantitative cross-code performance benchmark
remains in the manuscript.}

The reason is construct validity, not instrumentation: the local
\texttt{OlhoffApproach} performs a trial eigensolve after every MMA update
(roughly doubling its eigensolves per outer iteration) and adds a Heaviside
projection, a continuation schedule, and a grayness penalty absent from the
published method, while the local Yuksel-inspired code does not reproduce the
published iteration counts.  Benchmarking against either would be a strawman.

Consequently, every instruction below to rerun timing benchmarks, add
initialization/per-iteration columns to Table~1, state the conditions under which
the method is faster, explain the Yuksel 180--200 iteration discrepancy, or
recompute a scaling fit is \textbf{withdrawn}.  Those reviewer demands are
\emph{resolved by retraction}, not by evidence.  The efficiency argument survives
only as an operation count (one eigensolve at initialization versus one per design
iteration).  The frozen-eigenpair accuracy and cost question is answered
within a single implementation by \textbf{A4}.

The active campaign is \texttt{S1} $\to$ \texttt{EXP2} $\to$ \texttt{EXP2b} $\to$
\texttt{EXP3} $\to$ \texttt{A4}.
\end{tcolorbox}
...
A6 & R3 Gen.  & Scope efficiency claims to the tested local implementations and separate convergence from per-iteration cost & \urgent{Urgent} \\
\midrule
...
\textbf{Comparator scope.}  If Table~1 retains an Olhoff-related row, it
must be generated by \texttt{analysis/OlhoffApproach} and labelled
\emph{local Olhoff-inspired comparison implementation}.  Its timings and
iteration counts describe that code only; they are not measurements of a
canonical Du--Olhoff implementation.  Archived reconstruction outputs must
not enter the table.
...
\textbf{What needs to change.}
\begin{itemize}[leftmargin=1.6em]
  \item In the abstract, replace any blanket speedup claim with a statement
        about the tested local implementations.  The proposed implementation
        performs no eigensolve inside its iteration loop.  A numerical
        speedup may be stated only for an accepted production comparison and
        must name its comparator.  For an Olhoff-related comparison, that
        comparator is exclusively the local Olhoff-inspired
        \texttt{analysis/OlhoffApproach}, not canonical Du--Olhoff.
  ...
  \item Remove every frequency-gap, convergence, speedup, or scaling
        inference that treats an archived reconstruction as Du--Olhoff.
        There is no replacement canonical comparison.  This correction does
        not require a new experiment; it narrows the interpretation of the
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item Add a clarifying sentence in Section~3.2: explain which of the two
        ...
        this phenomenon.
  \item If they are spurious modes: \textbf{do not attribute them to the mass
        interpolation exponent.}  Per
        \texttt{MASS\_INTERPOLATION\_DECISION.md} (2026-07-14), the proposed
        method uses a \emph{linear} mass interpolation with $\rho_\mathrm{min}$
        regularization and \emph{no} low-density correction branch; the earlier
        statement that it uses $d=p=3$ was a manuscript error and is withdrawn.
        The local Olhoff-inspired comparison implementation does use a
        low-density branch.  Three mass models have been tested on the same
        400$\times$50 case (linear, $x^6$, and Du--Olhoff Eq.~4b) and
        \emph{none} removes the localized-mode family.  Current evidence
        instead associates these modes with disconnected solid components:
        they carry negligible kinetic energy in the low-density material and
        concentrate on solid regions not connected to the supports.  State this
        as current evidence, not as an established mechanism.  Reviewer~3's
        comment~\#11 suggestion about RAMP (see B4 below) may still be
        discussed, but it must not be presented as the identified cause.
\end{itemize}
...
\begin{itemize}[leftmargin=1.6em]
  \item Add a note in Section~3.2 explaining that the high mode indices
        ...
        of many spurious localized modes populating the lower end of the
        spectrum.  \textbf{Do not attribute them to the mass exponent}
        (\texttt{MASS\_INTERPOLATION\_DECISION.md}): the method uses a linear
        mass interpolation, and linear, $x^6$, and Du--Olhoff Eq.~4b were all
        tested without removing the family.  Current evidence associates these
        modes with disconnected solid components.
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item In the paper, acknowledge that spurious localized modes are present in
        the post-convergence spectra.  \textbf{Superseded framing:} this item
        formerly asked to state that ``SIMP with $d=p=3$ for the mass generates''
        them.  Per \texttt{MASS\_INTERPOLATION\_DECISION.md} the proposed method
        uses a \emph{linear} mass interpolation, and the mass exponent is
        \emph{not} the identified cause --- three mass models were tested and
        none removes the family.  The local Olhoff-inspired implementation does
        use a low-density branch, and that contrast may still be drawn, but not
        as an explanation of our spectra.  State that MAC
        tracking provides a robust mechanism to identify targeted modes
        independent of spectral pollution.  RAMP or $d>p$ may be discussed as a
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item State that adding a Heaviside continuation schedule (as in the
        local Olhoff-inspired comparison implementation) would sharpen the black-and-white
  ...
  \item If time permits, run the clamped-beam example with Heaviside
        ...
        local \texttt{OlhoffApproach} path; if MATLAB is used for the paper results, forward the
...
reported runs: CPU model, number of cores used (serial or parallel), available
RAM, operating system, and MATLAB version (MATLAB 2025b is already mentioned
on line~597).

\textbf{This demand survives the EXP1/EXP5 retirement.}  It is a
platform-disclosure requirement, not a performance claim: the master runner
prints and manifests the hardware/software specification for every stage,
independently of any timing measurement.  There is no Table~1 to anchor it to
--- the performance table is removed --- so the platform statement must simply
match the implementation that produced the \emph{reported} results (the
MATLAB campaign).

\textbf{Audit note:} do not state Python/NumPy/SciPy versions unless results
are actually regenerated with the Python implementation.  Do \emph{not} use
\texttt{examples/Performance/performance\_comparison.m} as the basis of this
statement: it is a legacy cross-code timing driver, it is not a campaign stage,
and it is not reviewer evidence.
...
A2 & Tauzowski & Add init-time column to Table~1; rerun timing for the local comparison set only & Week 1 \\
...
A6 & Tauzowski & Restrict efficiency claims to named local implementations in abstract and Section~4 & Week 1 \\
\midrule
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Timing instrumentation}
        ...
        the proposed MATLAB solver.  The Olhoff-related production row must
        invoke only \texttt{analysis/OlhoffApproach}; no archived
        reconstruction path is a timing target.
...
Algorithm~1 is a conceptual summary of the published Olhoff--Du formulation;
it is literature background, not a validated executable baseline in this
repository.  Empirical comparisons must instead name the tested repository
implementations.  In particular, the sole Olhoff-related production
comparator is \texttt{analysis/OlhoffApproach}, labelled the local
Olhoff-inspired comparison implementation.  It adds a trial eigensolve after
each MMA update, while the local Yuksel timing aggregates Stage~1 and Stage~2
iterations.  These implementation choices must be stated explicitly
wherever empirical runtime or convergence ratios are discussed.  No property
or result from an archived reconstruction may be transferred to
Algorithm~1.
...
% -----------------------------------------------------------
\subsection{Algorithm 1: Published Olhoff--Du Bound Formulation
(Literature Background)}
\label{subsec:alg-olhoff}
% -----------------------------------------------------------

As described in the literature, the method maximizes a scalar bound variable
$\beta \leq \omega_j^2$,
...
\cite{Olhoff2014}.  This pseudocode supports conceptual discussion only and
must not be presented as the algorithm executed for repository timing or
...
\begin{table}[htbp]
\centering
\caption{Per-iteration operation count for the tested repository
implementations.
$bw \approx \sqrt{N_e}$ denotes the effective bandwidth;
$J$ is the number of tracked modes;
$c = r_{\min}/h$ is the filter radius in elements (constant).  The first
column describes only \texttt{analysis/OlhoffApproach}, not the published
algorithm or an exact reconstruction.}
\label{tab:ops}
\renewcommand\arraystretch{1.2}
\begin{tabularx}{\textwidth}{@{}lXXX@{}}
\toprule
Operation & Local Olhoff-inspired implementation & Local Yuksel--Yilmaz (Stage~2) & Proposed \\
\midrule
...
\subsubsection*{Empirical comparison policy}

\textbf{Superseded (13 July 2026): there is no performance table.}  EXP1 and
EXP5 are retired, planning items A2/A6 are void, and no cross-code performance
benchmark remains in the manuscript.  The relationship
\[
  T = n_{\mathrm{iter}}\,t_{\mathrm{iter}}
\]
is retained only as a qualitative decomposition; it is not populated with
measurements for any comparator.

The following interpretations are withdrawn and must not appear in the
manuscript, response letter, figures, captions, or supplementary material:
\begin{itemize}[leftmargin=1.6em]
  \item a frequency gap to a Du--Olhoff optimum;
  \item speedup relative to canonical or exactly reproduced Du--Olhoff;
  \item convergence-rate comparisons attributed to the published method;
  \item scaling claims derived from any \texttt{OlhoffApproachExact} output; and
  \item any runtime, memory, speedup, or scaling comparison against the local
        \texttt{OlhoffApproach} or the local Yuksel-inspired implementation,
        which are not faithful reference implementations.
\end{itemize}
No replacement experiment is introduced.  Archived reconstruction results and
the archived EXP1/EXP5 artifacts are excluded from every numerical aggregation.

\subsubsection*{Permitted complexity interpretation}

Algorithm~1 documents the structure reported in the literature.  The
operation counts in Table~\ref{tab:ops}, by contrast, describe the local
\texttt{analysis/OlhoffApproach} execution path, including its trial
eigensolve.  They support a qualitative explanation of that local code's
cost, not a claim about the runtime constant, convergence, accuracy, or
scaling of canonical Du--Olhoff.

Consequently, the revised discussion may compare setup cost,
per-iteration cost, iteration count, total runtime, and memory only after
naming the local implementations that produced those quantities.  It may
also discuss the proposed and local Yuksel implementations on the same
basis.  It must not transfer empirical properties of the local
Olhoff-inspired comparator to the published formulation, describe its
endpoint as a reference optimum, or use it to validate a frequency gap.
