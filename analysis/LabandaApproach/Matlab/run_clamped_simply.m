% run_clamped_simply.m — Clamped-simply beam benchmark for TopIQP
%
% Geometry and boundary conditions are ported from the Olhoff-inspired
% frequency-maximization example:
%   analysis/OlhoffApproach/Matlab/run_clamped_simply.m
%   analysis/OlhoffApproach/Matlab/topFreqOptimization_MMA.m (buildSupports)
%
% That source problem is an unloaded eigenvalue problem (L=8, H=1,
% 240x30 mesh, left edge fully clamped, right edge pinned at mid-height).
% TopIQP minimises compliance, so there is no load to reuse; a top-edge,
% mid-span point load fy=-1 (three-point bending) is added here — see
% topIQP.m bcType='cs'.
%
...
fprintf('=== Clamped-Simply 240x30, volfrac=0.5 (method=%s) ===\n', method);
...
figure('Name','Clamped-Simply 240x30 V=0.5','Color','white');
imagesc(1 - r.xPhys); colormap(gray); axis equal off;
title(sprintf('Clamped-Simply 240\\times30,  V=0.5,  c=%.4g,  iter=%d', r.compliance, r.nIter));
