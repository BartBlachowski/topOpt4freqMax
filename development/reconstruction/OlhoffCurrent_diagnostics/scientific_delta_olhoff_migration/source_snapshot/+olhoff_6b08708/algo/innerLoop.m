...
%     volFun   OPTIONAL handle [V,gradV] = volFun(dx), supplied only by the
%              projection path of olhoffOpt.  When present, ctx.rho holds the
%              OPTIMIZATION variable z (not the density) and ctx.rhomin its
%              lower box bound, so the box code below bounds dz; the volume
%              constraint is then evaluated through volFun instead of the
%              linear expression.  Absent -> frozen behaviour, bitwise.
...
% Optional exact volume evaluator, supplied ONLY by the projection path of
% olhoffOpt (audit_m4_projection_invariance).  Absent -> the frozen linear
% volume constraint below is executed verbatim and the trajectory is bitwise
% identical.
useVolFun = isfield(ctx,'volFun') && ~isempty(ctx.volFun);
...
for it = 1:ctx.maxInner
    ...
    % (25e)  volume
    if useVolFun
        % Projection path: V is a nonlinear function of the optimization
        % variable, so value and gradient are formed as a consistent pair by
        % re-applying the density filter and the projection at the trial point
        % z+dz.  This preserves the frozen code's EXACT evaluation of (25e) at
        % every sub-iterate (the branch below is exact only because V is linear
        % there).  See WP2_PROJECTION_MATH.md sec.4.3.
        [Vnow, gV]   = ctx.volFun(drho);
        fval(N+2)    = (Vnow - Vtot)/Vtot;
        dfdx(N+2,1:NE) = gV(:).'/Vtot;
    else
