function res=olhoffOptStabilized(cfg,policy)
%OLHOFFOPTSTABILIZED Audit-only mirror with a causal move-continuation policy.
% Numerical FE, modal, gradient, filter and LP decisions are copied from the
% frozen Du-Olhoff reproduction. The only experimental action is replacing
% cfg.move by a predeclared nonincreasing sequence after a native online event.
