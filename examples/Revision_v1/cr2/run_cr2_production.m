function run_cr2_production()
%RUN_CR2_PRODUCTION Execute and accept/reject the unchanged CR2 A/B configs.
...
configAPath = fullfile(scriptDir, 'cr2_variant_a_omitted.json');
configBPath = fullfile(scriptDir, 'cr2_variant_b_complete.json');
validatorPath = fullfile(repoRoot, 'scripts', 'revision_v1', 'validate_v1c_cr2_configs.py');
validationPath = fullfile(outputDir, 'v1c_cr2_production_validation.json');
summaryJsonPath = fullfile(outputDir, 'cr2_production_results.json');
summaryMdPath = fullfile(outputDir, 'cr2_production_summary.md');
manifestPath = fullfile(outputDir, 'cr2_production_manifest.json');
logPath = fullfile(outputDir, 'cr2_production_run.log');
...
fprintf('CR2 production run started: %s\n', char(datetime('now', 'TimeZone', 'UTC')));
fprintf('Configs are executed unchanged. No A4, Exp2, or Exp3 path is invoked.\n');
...
fprintf('%s', output);
if status ~= 0
    error('CR2Production:V1cFailed', 'V1c config validation failed with status %d.', status);
end
validation = jsondecode(fileread(validationPath));
if ~strcmp(char(validation.status), 'passed')
    error('CR2Production:V1cResult', 'V1c config validation did not pass.');
end

criteria = struct('design_change_tolerance', 1e-3, ...
    'feasibility_tolerance', 1e-8, 'mac_threshold', 0.8, ...
...
gatePassed = variantA.acceptance.accepted && variantB.acceptance.accepted && ...
    differenceArtifacts.complete;
if gatePassed
    gateStatus = 'passed';
    claimStatus = 'eligible_for_scientific_analysis_no_automatic_claim';
else
    gateStatus = 'failed_partial';
    claimStatus = 'withheld';
end

summary = struct();
summary.study = 'CR2 omitted-versus-complete authoritative load sensitivity';
summary.gate = 'E4-CR2';
summary.status = gateStatus;
summary.scientific_claim = claimStatus;
summary.authoritative_load = 'F(x) = omega0^2 * M(x) * Phi0';
summary.acceptance_criteria = criteria;
summary.v1c_validation = validation;
...
localWriteManifest(manifestPath, repoRoot, scriptDir, outputDir, ...
    ...
    summaryMdPath, logPath, variantA, variantB, differenceArtifacts, gateStatus);

fprintf('\nCR2 PRODUCTION GATE: %s\n', upper(gateStatus));
...
fprintf('  Scientific claim: %s\n', claimStatus);
fprintf('  Summary: %s\n', summaryMdPath);
fprintf('  Manifest: %s\n', manifestPath);
diary('off');

if ~gatePassed
    error('CR2Production:GateFailed', ...
        'Gate E4 CR2 is failed/partial. Diagnostics and artifacts were preserved.');
end
end
...
fprintf(fid,'# CR2 Production Summary\n\n');
fprintf(fid,'- Gate E4 CR2 status: **%s**\n', upper(summary.status));
fprintf(fid,'- Scientific claim: `%s`\n', summary.scientific_claim);
fprintf(fid,'- Load: `F(x) = omega0^2 * M(x) * Phi0`\n');
fprintf(fid,'- Acceptance: design change <= 1e-3, feasibility <= 1e-8, MAC >= 0.8, not capped, all artifacts present.\n\n');
...
if ~strcmp(summary.status,'passed')
    fprintf(fid,'No scientific effect claim is permitted because both variants were not accepted.\n');
end
end

function localWriteManifest(path, repoRoot, scriptDir, outputDir, configA, configB, validator, validation, summaryJson, summaryMd, logPath, a, b, diffArtifacts, gateStatus)
paths = {fullfile(scriptDir,'run_cr2_production.m'),configA,configB,validator,validation, ...
...
roles = {'production runner','Variant A config','Variant B config','V1c validator','V1c result', ...
...
manifest=struct('study','CR2 production','gate','E4-CR2','status',gateStatus, ...
