...
%                telemetry.stopping.status, the precedence-ordered verdict
%                (SOLVER_FAILURE > CONVERGED > CAP_HIT > RUNNING) beside the
...
%                NaN where it does not.
    ...
    % Effective solver-side configuration, as the solver itself reports having
    % executed.  Populated only by dispatch cases whose solver exposes one; it
    % is what the Olhoff benchmark-path equivalence gate hashes, so that a
    % configuration-mapping change cannot pass unnoticed.  See
    ...
    telemetry.timing = struct( ...
        ...
        ... % Component split, where the solver reports one.  NaN -- not zero --
        ... % when it does not: zero would read as "spends no time solving
    ...
    telemetry.stopping = struct( ...
        ...
        ... % benchmark can check that itself instead of trusting stop_reason.
        'status', telemetryValue(solverStopping, 'status', 'N/A'), ...
        'native_stop_reason', ...
            telemetryValue(solverStopping, 'native_stop_reason', 'N/A'), ...
        'native_break_taken', ...
            telemetryValue(solverStopping, 'native_break_taken', NaN), ...
        'final_multiplicity', ...
            telemetryValue(solverStopping, 'final_multiplicity', NaN), ...
        'subproblem_failed', ...
            telemetryValue(solverStopping, 'subproblem_failed', false), ...
        'n_subproblem_failures', ...
            telemetryValue(solverStopping, 'n_subproblem_failures', NaN), ...
        'final_inner_converged', ...
            telemetryValue(solverStopping, 'final_inner_converged', NaN), ...
        'final_lp_flag', telemetryValue(solverStopping, 'final_lp_flag', NaN), ...
        'lp_failure_iters', ...
            telemetryValue(solverStopping, 'lp_failure_iters', []));
    telemetry.solver_config = solverConfig;
    ...
    telemetry.yuksel = struct( ...
        ...
        'stage1_loop_time', telemetryValue(solverTiming, 'stage1_loop_time', NaN), ...
        'stage2_loop_time', telemetryValue(solverTiming, 'stage2_loop_time', NaN), ...
        'total_loop_time', telemetryValue(solverTiming, 'total_loop_time', NaN));
