function [drho, beta_final, hist] = inner_loop_mma_lpdiag(rho, lambda_bar, fsk, ...
                                                     lambda_J, dlam_J, ...
                                                     volfrac, rho_min, ...
...
%              Eq. 15-19, 22-23, 25.  Svanberg (1987) MMA.
if nargin < 12 || isempty(subproblem_formulation)
    subproblem_formulation = 'nonlinear';
end
subproblem_formulation = lower(strtrim(char(subproblem_formulation)));
if ~ismember(subproblem_formulation, {'nonlinear','lp_reduced'})
    error('inner_loop_mma_lpdiag:InvalidSubproblemFormulation', ...
        'subproblem_formulation must be ''nonlinear'' or ''lp_reduced''.');
end
...
if strcmp(subproblem_formulation, 'lp_reduced')
    offdiag_pairs = N * (N - 1) / 2;
    m = N + 2*offdiag_pairs + 1 + has_J; % diag cluster + +/- offdiag + volume + optional J
else
...
hist.offdiag_max  = nan(inner_max_iter, 1);
hist.n_iters      = 0;
...
for inner_it = 1:inner_max_iter
    ...
    if strcmp(subproblem_formulation, 'lp_reduced')
        % LP-reduced diagnostic path from paper Eq. 22-23:
        %   f_sk^T Delta_rho = 0 for s ~= k
        %   Delta lambda_j = f_jj^T Delta_rho
        % Equality constraints are represented as paired inequalities because
        % this diagnostic keeps MMA as the optimizer.
        row = 1;
        for i = 1:N
            fii = fsk(:, i, i);
            mu_i = fii' * Delta_rho;
            fval(row) = beta_hat - 1 - mu_i / lambda_ref;
            dfdx(row, 1) = 1;
            dfdx(row, 2:end) = -fii' / lambda_ref;
            row = row + 1;
        end
        for s = 1:N
            for k = (s+1):N
                foff = fsk(:, s, k);
                off = (foff' * Delta_rho) / lambda_ref;
                fval(row) = off;
                dfdx(row, 1) = 0;
                dfdx(row, 2:end) = foff' / lambda_ref;
                row = row + 1;
                fval(row) = -off;
                dfdx(row, 1) = 0;
                dfdx(row, 2:end) = -foff' / lambda_ref;
        ...
        % Nonlinear subeigenvalue path from paper Eq. 18/25d:
        %   Delta lambda_i = eig_i(F(Delta_rho)).
    ...
    %   sum(rho + Delta_rho)/nEl - volfrac <= 0
    ...
    hist.offdiag_max(inner_it)  = offdiag_abs;
...
if N > 1
    hist.final_offdiag_max = max(abs(F_mat(~eye(N))));
else
    hist.final_offdiag_max = 0;
end
end
