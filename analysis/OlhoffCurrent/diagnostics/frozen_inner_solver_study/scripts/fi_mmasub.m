...
fi_mmasub(m,n,iter,xval,xmin,xmax,xold1,xold2, ...
f0val,df0dx,fval,dfdx,low,upp,a0,a,c,d,auditCfg)
%
...
epsimin = auditCfg.epsimin;
raa0 = 0.00001;
move = 0.5;
albefa = 0.1;
asyinit = auditCfg.asyinit;
asyincr = 1.2;
asydecr = 0.7;
eeen = ones(n,1);
eeem = ones(m,1);
...
if iter < 2.5
  ...
  lowmin = xval - auditCfg.asymax*(xmax-xmin);
  lowmax = xval - 0.01*(xmax-xmin);
  uppmin = xval + 0.01*(xmax-xmin);
  uppmax = xval + auditCfg.asymax*(xmax-xmin);
  low = max(low,lowmin);
  low = min(low,lowmax);
  upp = min(upp,uppmax);
  upp = max(upp,uppmin);
