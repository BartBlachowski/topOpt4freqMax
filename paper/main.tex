...
\begin{abstract}
\internallinenumbers
Topology optimization for maximizing natural frequencies is important for resonance-safe design, but repeated modal analyses make standard formulations expensive. This paper introduces a single-step static method that approximates the topology maximizing a selected frequency by solving a linear system instead of the generalized eigenvalue problem. Compared with the two-step static strategy of Yuksel and Yilmaz (2025), the proposed approach avoids sequential modal estimates and reduces algorithmic complexity.

The method is assessed on three benchmarks: a simply supported beam (against Du and Olhoff (2007) and Yuksel and Yilmaz (2025)), a clamped beam for multi-mode control, and a building-like domain with passive regions. Using a load-weighting parameter $\alpha$ and Modal Assurance Criterion (MAC) checks, results show that quasi-static loads proportional to $\boldsymbol{\Phi}_j$ produce monotonic gains in the corresponding mode frequency across tested cases. For a $320 \times 40$ mesh, the method is about 8.6 times faster than Olhoff and 2.5 times faster than the Yuksel dynamic implementation, while using 21~MB RAM versus 163~MB and 44~MB, respectively. The proposed formulation provides a practical low-cost route for rapid vibration-oriented structural design.
\end{abstract}
...
0 < x_{\min} \le x_e \le 1 &, \qquad e = 1,\ldots,n_e.
\end{align*}
The inertial forces acting on the converged topology are shown in Fig.~\ref{fig:third}.
...
\textbf{Step 4.} Perform optimization from Step 3 modifying at each iteration the load vector making it depend on the current values of design variables $\mathbf{M}(\mathbf{x})$. Continue this static optimization until convergence is achieved. The resulting suboptimal topology is shown in Fig.~\ref{fig:forth}.
...
Three benchmark structures are examined. The first is a simply supported beam, used to compare the proposed method against the Olhoff and Yuksel approaches. The second is a clamped beam, used to illustrate multi-mode frequency control via the load-weighting parameter $\alpha$. The third is a building-like structure, used to demonstrate the method's applicability to problems with passive elements and more complex geometry. For the clamped-beam and building examples, MAC correlation analysis is used to track how each initial solid-domain mode shape relates to the modes of the final topology, enabling a mode-shape-aware assessment of the frequency gains achieved by each $\alpha$ configuration.
...
\begin{table}[H]
\centering
\caption{First two eigenfrequencies (conventional eigenvalue ordering) for the five topologies in the clamped-beam example. Initial solid-domain frequencies: $\omega_1^{(0)}=145.44$~rad/s, $\omega_2^{(0)}=362.67$~rad/s.}
...
\begin{table}[H]
\centering
\caption{MAC-tracked mode frequencies for the clamped-beam example. For each topology, the mode with the highest MAC value relative to the solid-domain shape $\boldsymbol{\Phi}_j$ is identified and its frequency $\tilde{\omega}_j$ and position in the final eigenvalue ordering are reported.}
\label{tab:clampedBeamMAC}
\resizebox{\textwidth}{!}{%
\begin{tabular}{ccccccc}
\Xhline{1.2pt}
$\alpha$ & MAC-best mode for $\boldsymbol{\Phi}_1$ & $\tilde{\omega}_1$ [rad/s] & gain & MAC-best mode for $\boldsymbol{\Phi}_2$ & $\tilde{\omega}_2$ [rad/s] & gain \\
\Xhline{1.2pt}
1.00 & 3 & 368.2 & $2.53\times$ & 46 & 722.0 & $1.99\times$ \\
0.75 & 1 & 347.7 & $2.39\times$ & 30 & 629.1 & $1.73\times$ \\
0.50 & 1 & 318.9 & $2.19\times$ & 18 & 744.5 & $2.05\times$ \\
0.25 & 1 & 254.0 & $1.75\times$ & 40 & 816.0 & $2.25\times$ \\
0.00 & 1 & 113.3 & $0.78\times$ & 91 & 888.5 & $2.45\times$ \\
\Xhline{1.2pt}
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{First two eigenfrequencies (conventional eigenvalue ordering) for the five topologies in the building example. Initial solid-domain frequencies: $\omega_1^{(0)}=19.84$~rad/s, $\omega_2^{(0)}=90.93$~rad/s.}
...
\begin{table}[H]
\centering
\caption{MAC-tracked mode frequencies for the building example. For each topology, the mode with the highest MAC value relative to the solid-domain shape $\boldsymbol{\Phi}_j$ is identified and its frequency $\tilde{\omega}_j$ and position in the final eigenvalue ordering are reported.}
...
$\alpha$ & MAC-best mode for $\boldsymbol{\Phi}_1$ & $\tilde{\omega}_1$ [rad/s] & gain & MAC-best mode for $\boldsymbol{\Phi}_2$ & $\tilde{\omega}_2$ [rad/s] & gain \\
\Xhline{1.2pt}
1.00 & 3 &  91.4 & $4.61\times$ & 33 & 169.8 & $1.87\times$ \\
0.75 & 3 &  88.9 & $4.48\times$ & 58 & 222.8 & $2.45\times$ \\
0.50 & 1 &  66.9 & $3.37\times$ & 81 & 281.3 & $3.09\times$ \\
0.25 & 1 &  52.0 & $2.62\times$ & 69 & 302.2 & $3.32\times$ \\
0.00 & 1 &  33.6 & $1.69\times$ & 71 & 324.5 & $3.57\times$ \\
\Xhline{1.2pt}
\end{tabular}%
}
\end{table}
