"""
Run the Clamped-Hinged Beam frequency-maximization example.

Configuration: ClampedHingedBeamTopOptFreq.json
    - 8 x 1 m beam, clamped left (vertical line x=0), hinged right (closest point [8,0.5])
...
JSON_FILE = os.path.join(_HERE, "ClampedHingedBeamTopOptFreq.json")

if __name__ == "__main__":
    x, omega, t_iter, n_iter = run_topopt_from_json(JSON_FILE)
    print(f"\nClamped-Hinged Beam done.")
