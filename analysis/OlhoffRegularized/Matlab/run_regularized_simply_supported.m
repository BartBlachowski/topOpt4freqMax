%% OlhoffRegularized - simply supported 8 x 1 beam
...
nelx=400;nely=50;volfrac=.5;penal=3;rmin=2.5;move=.005;
bcType="simply";
...
localPlot(rho,omega,info,nelx,nely,'Simply supported');
localReport(omega,info,nelx,nely,rmin);

function localPlot(rho,omega,info,nelx,nely,label)
figure('Name','OlhoffRegularized - simply supported','Color','w');
tiledlayout(2,1,'TileSpacing','compact','Padding','compact');
nexttile;imagesc(1-reshape(rho,nely,nelx));set(gca,'YDir','normal');
axis equal tight off;colormap(gray(256));clim([0 1]);
title(sprintf('%s: \\omega_1 = %.2f rad/s',label,omega(1)),'Interpreter','tex');
...
fprintf('\nOlhoffRegularized -- simply supported\n');
