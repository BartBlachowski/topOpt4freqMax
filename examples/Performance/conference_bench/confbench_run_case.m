...
%                optimization iteration) and nothing else; the preparation
%                around it is overhead, like every other method's setup
%                (CONFBENCH_PROPOSED_TIMES).  Stage 2 is the SIMP topology
%                optimization.
...
% (empty if the run was not the recorded production preset), effective
% configuration hash, promoted source tree hash, repository commit.
...
rec.times = struct( ...
    ...
    'outer_time_total_s', a.outer_time_total_s, ...
    'outer_time_per_outer_mean_s', a.outer_time_per_outer_mean_s, ...
    'outer_time_per_outer_median_s', a.outer_time_per_outer_median_s, ...
    'outer_time_excluding_inner_per_outer_mean_s', a.outer_time_excluding_inner_per_outer_mean_s, ...
    'eigen_time_per_outer_mean_s', a.eigen_time_per_outer_mean_s, ...
    'gradient_time_per_outer_mean_s', a.gradient_time_per_outer_mean_s, ...
    'total_wall_time_per_outer_s', a.total_wall_time_per_outer_s, ...
...
% Time 1 = the reference eigenanalysis alone; the preparation goes to
% overhead_time_s (the table's Other column) like every other method's setup.
% The nesting tEig <= t1 is asserted inside the helper.
rec.times = confbench_proposed_times(t1, tEig, t2, callWall, tel.timing.total_wall_time);
end
