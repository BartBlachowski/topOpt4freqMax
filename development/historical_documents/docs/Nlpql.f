      SUBROUTINE NLPQL0(M,ME,N,X,F,XL,XU,IPRINT,IFAIL,WA,LWA,KWA,LKWA)
C
C***********************************************************************
C
C
C        AN IMPLEMENTATION OF A SEQUENTIAL QUADRATIC PROGRAMMING
C          METHOD FOR SOLVING NONLINEAR OPTIMIZATION PRPOBLEMS
C
C
C
...
C                         XL  <=  X  <=  XU
C
C   NLPQL0 IS THE EASY-TO-USE SUBROUTINE TO SOLVE THE ABOVE NONLINEAR
C   OPTIMIZATION PROBLEM. IF SOME PARAMETERS ARE TO BE CHANGED, IF
C   MORE INITIAL INFORMATION SHALL BE PROVIDED, IF REVERSE COMMUNI-
C   CATION IS TO BE USED, OR IF PROGRAM MODULES ARE TO BE EXCHANGED,
C   PROCEED TO SUBROUTINE NLPQL1.
C
C
C   USAGE:
C
C      NLPQL0(M,ME,N,X,F,XL,XU,IPRINT,IFAIL,WA,LWA,KWA,LKWA)
C
C
...
C      IPRINT = 2 :  ONE LINE OF INTERMEDIATE RESULTS ARER PRINTED IN EACH
C                    ITERATION.
...
C      IFAIL = 1 :   THE ALGORITHM HAS BEEN STOPPED AFTER 40 ITERATIONS.
...
C      IFAIL = 4 :   MORE THAN 10 FUNCTION CALLS ARE REQUIRED DURING THE
C                    LINE SEARCH.
...
C             AT LEAST N*N + M*N + 11*M + 25*N + 60 AND HAS TO BE ENLARGED,
C             IF ADDITIONAL WORKING SPACE IS REQUIRED TO SOLVE THE
C             QUADRATIC PROGRAMMING OR LINEAR LEAST SQUARES SUBPROBLEM.
C   KWA(LKWA),LKWA :  THE USER HAS TO PROVIDE WORKING SPACE FOR AN
C             INTEGER ARRAY. LKWA SHOULD BE AT LEAST 18 OR GREATER,
C             IF ADDITIONAL SPACE IS REQUIRED TO SOLVE THE SUBPROBLEM.
C
C   ALL CALCULATIONS WITHIN THE SUBROUTINES OF THIS FILE ARE PERFORMED
C   IN DOUBLE PRECISION ARITHMETIC.
C   A NAMED COMMON-BLOCK  /CMACHE/EPS  HAS TO BE DEFINED BY THE USER
C   CONTAINING A GUESS FOR THE UNDERLYING MACHINE PRECISION. ONE FURTHER
...
C                     ACTIVE(J) = .TRUE.) AND TO STORE THE RESULTS IN G
C                     (I.E. G(J)). IF ANALYTICAL DIFFERENTIATION IS USED,
C                     THEN ALL CONSTRAINTS MUST BE EVALUATED SIMULTANEOUSLY.
...
C            OBJECTIVE AND CONSTRAINTS AT X
C
C
...
C
C
C   VERSION:  2.3    (APRIL,1987)
C
C
C***********************************************************************
C
      INTEGER N,LWA,LKWA
      DIMENSION X(N),XL(N),XU(N),WA(LWA),KWA(LKWA)
      INTEGER M,ME,IPRINT,IFAIL,KWA
      INTEGER MAXFUN,MAXIT,MODE,IOUT,N1,M1,MNN1,MNN2,IG,IDF,IDG,IU,
     1     IDCL,ICD,IWA,LWA1,LACTIV,ITEST
      LOGICAL ACTIVE(2000),LQL,LMERIT
      DOUBLE PRECISION X,F,XL,XU,WA,ACC,SCBOU,EPS
C
C   INTRINSIC FUNCTION:  DSQRT
C
      COMMON/CMACHE/EPS
C
C   PREPARE SOME DATA FOR CALLING NLPQL1
C
      EPS = 1.0D-14
      ACC=DSQRT(EPS)
      SCBOU=1.D+3
...
C
      ...
      LWA1=LWA-IWA+1
      LACTIV=2000
      ITEST=IWA+6*M+20*N+60
      IF(ITEST.LE.LWA) GOTO 1
      IFAIL=5
      IF(IPRINT.EQ.0) RETURN
      WRITE(IOUT,1000) ITEST
 1000 FORMAT(8X,44H**LENGTH OF WA TOO SHORT. SHOULD BE AT LEAST,I6)
      RETURN
    1 CONTINUE
C
C   CALL OF NLPQL1
C
      CALL NLPQL1(M,ME,M1,N,N1,MNN2,X,F,WA(IG),WA(IDF),WA(IDG),WA(IU),
     1     XL,XU,WA(IDCL),WA(ICD),ACC,SCBOU,MAXFUN,MAXIT,IPRINT,MODE,
     2     IOUT,IFAIL,WA(IWA),LWA1,KWA,LKWA,ACTIVE,LACTIV,LMERIT,LQL)
C
C   END OF NLPQL0
C
      RETURN
      END
      SUBROUTINE NLPQL1(M,ME,MMAX,N,NMAX,MNN2,X,F,G,DF,DG,U,XL,XU,
     1     C,D,ACC,SCBOU,MAXFUN,MAXIT,IPRINT,MODE,IOUT,IFAIL,WA,LWA,
     2     KWA,LKWA,ACTIVE,LACTIV,LMERIT,LQL)
C
C***********************************************************************
C
C
...
C   RESPECT TO AN AUGMENTED LAGRANGE MERIT FUNCTION TO OBTAIN THE NEW
C   ITERATE.
C
...
C             IS DIVIDED BY THE INVERSE OF THE SQUARE ROOT. IF SCBOU IS
...
C
C
C   TO EXECUTE NLPQL0 OR NLPQL1, THIS FILE MUST BE COMPILED AND LINKED THE
...
C***********************************************************************
C
      INTEGER N,MMAX,NMAX,MNN2,LWA,LKWA,LACTIV
      DIMENSION X(N),G(MMAX),DF(NMAX),DG(MMAX,NMAX),U(MNN2),
     1     XL(N),XU(N),C(NMAX,NMAX),D(NMAX),WA(LWA),
     2     KWA(LKWA),ACTIVE(LACTIV)
...
C
 ...
 1000 FORMAT(////,3X,
     ...
     5 60H------------------------------------------------------------,
     6 15H---------------)
CCCCC WRITE(IOUT,1100) MODE,ACC,SCBOU,MAXFUN,MAXIT,IPRINT
 1100 FORMAT(/,5X,11HPARAMETERS:,/,8X,6HMODE =,I3,/,8X,5HACC =,
     ...
     2     I5,/,8X,8HIPRINT =,I4)
      ...
      WRITE(IOUT,1600)
 1600 FORMAT(/,5X,30HOUTPUT IN THE FOLLOWING ORDER:,
     1   /,8X,24HIT    - ITERATION NUMBER,
     2   /,8X,32HF     - OBJECTIVE FUNCTION VALUE,
     3   /,8X,35HSCV   - SUM OF CONSTRAINT VIOLATION,
     4   /,8X,36HNA    - NUMBER OF ACTIVE CONSTRAINTS,
     4   /,8X,40HI     - NUMBER OF LINE SEARCH ITERATIONS,
     5   /,8X,28HALPHA - STEPLENGTH PARAMETER,
     6   /,8X,52HDELTA - ADDITIONAL VARIABLE TO PREVENT INCONSISTENCY,
     7   /,8X,43HDLAN  - MAXIMUM NORM OF LAGRANGIAN GRADIENT,
     8   /,8X,40HKT    - KUHN-TUCKER OPTIMALITY CRITERION)
      WRITE(IOUT,1700)
 1700 FORMAT(//,2X,
     1 60HIT         F          SCV     NA  I    ALPHA    DELTA      D,
     2 15HLAN      KT    ,
     3 /,2X,
     4 60H------------------------------------------------------------,
     5 15H---------------)
...
C
 ...
 1400 FORMAT(8X,48H**LENGTH OF KWA TOO SHORT. SHOULD BE AT LEAST 19)
      RETURN
    7 IF(LACTIV.GT.2*MMAX+15) GOTO 8
      IFAIL=5
      IF(IPRINT.EQ.0) RETURN
      WRITE(IOUT,1500)
 1500 FORMAT(8X,
...
C*********************************************************************
C
      INTEGER N,MNN2,MNN,NMAX,MMAX,N1,NMNN,NO1,MO1,LWA,LIW,LACTIV
...
C
  ...
  205 ACTIVE(M+J)=.TRUE.
  206 CONTINUE
      CALL NLFUNC(M,ME,MMAX,N,F,G,X,ACTIVE(M+1))
      CALL NLGRAD(M,ME,MMAX,N,F,G,DF,DG,X,ACTIVE,WA(41))
   77 CONTINUE
      LSCF=.FALSE.
      LSCG=.FALSE.
...
C
 ...
 1510 FORMAT(8X,30HNUMBER OF ACTIVE CONSTRAINTS: ,19X,5HNAC =,I4)
   ...
   70 SUM=SUM + DABS(U(M+I)*V(I)) + DABS(U(MN+I)*W(I))
      ...
      FF=F
      IF (LSCF) FF=F/SCF
      WRITE(IOUT,1460) ITER,FF,SRES,NACT,ILINE,ALPHAO,DEL(N1),
     1     DLAN,SUM
 1460 FORMAT(1X,I3,D16.8,D10.2,I4,I3,4D10.2)
  169 CONTINUE
...
C
    ...
    2 ILINE=ILINE+1
      ...
      IFAIL=-1
      RETURN
    1 IF(ILINE.LT.MAXFUN) GOTO3
      IFAIL=1
      RETURN
    3 DIFF=PHI - WA(1)
      IF(DIFF.GT.AMUE*DPHI) GOTO4
      IFAIL=0
      RETURN
...
C
    3 IF(M.EQ.0) GOTO30
      DO31 J=1,M
      ACTIVE(M+J)=.TRUE.
   31 IF(J.GT.ME.AND.G(J).GT.V(J)/R(J)) ACTIVE(M+J)=.FALSE.
   30 CONTINUE
      IF(MODE.EQ.4) GOTO4
C
