...
No evaluator is privileged. The three evaluators form a **common evaluator family**: one
stiffness/mass interpolation pair per member, applied identically to every method's density
field regardless of which optimizer produced it.

E1 matches Proposed's interpolation up to floor values. **E2 and E3 no longer reproduce the
Yuksel and Olhoff native mass interpolations exactly.** Their stiffness laws and floors
remain each method's own, but their low-density mass branch uses the source-defined
continuous Du & Olhoff (2007) Eq. (4a), `g(x) = 1e5*x^6` for `x <= 0.1`, in place of the
native discontinuous Eq. (4), `g(x) = x^6`. Native identity in the common post-hoc
evaluator was deliberately sacrificed: exact native fidelity imported an
optimization-specific discontinuity of factor 1e5 at `x = 0.1` that is numerically unstable
at the trajectory-quality scale this study measures. See
`analysis/iteration_efficiency_evaluator_discontinuity_audit/` for the finding and
`analysis/iteration_efficiency_phase2d_evaluator_amendment/` for the amendment. The
optimizers themselves are unchanged and continue to use their native Eq. (4).

E2 and E3 still share the same piecewise mass law and differ only in stiffness floor, so the
three-evaluator minimum remains closer to two-way in evidential terms. Frozen endpoint
evidence mitigates but does not erase this symmetry issue: their raw omega1 values differ by
at most 0.429% and preserve the same ordering wherever all three evaluator values are
available. That 0.429% figure was measured under the pre-amendment evaluator and must be
recomputed before it is cited again.
...
- E1/E2/E3 Candidate C selected structural frequency and ordinal at entry and certification;
...
## Phase 2H common-evaluator amendment

The quality numerator and denominator use Candidate C actual-gray selected structural
frequencies. The exact-count binary spectrum is diagnostic-only. Evaluator search work is
offline common assessment work and is not charged as a native method iteration. Candidate
C precision, cross-method, and reference-length qualification are required before any
production endpoint is eligible for reporting.
