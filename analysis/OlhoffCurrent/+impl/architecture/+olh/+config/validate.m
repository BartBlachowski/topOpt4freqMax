...
% ---- the stage-exhaustion controller is one rule, not two half-rules ----
% E = A OR B descends the ladder AND, at the last level, admits convergence.
% Selecting the terminal half without the descending half leaves a declaration
% with nothing to consume it below the last rung, and the run could never stop.
if strcmp(g('stop.rule'),'stageExhaustion')
    if ~strcmp(g('move.policy'),'ladder')
        error('olh:config:exhaustionNeedsLadder', ...
            ['stop.rule=''stageExhaustion'' admits convergence only at the last ' ...
             'move.levels rung, so it requires move.policy=''ladder'' (got ''%s'').'], ...
            g('move.policy'));
    end
    if ~strcmp(g('move.continuation.signal'),'stageExhaustion')
        error('olh:config:exhaustionSplit', ...
            ['stop.rule=''stageExhaustion'' requires ' ...
             'move.continuation.signal=''stageExhaustion'' (got ''%s''): the same ' ...
             'frozen rule must govern both the stage transition and the terminal ' ...
             'admission, or a declaration below the last rung is never consumed.'], ...
            g('move.continuation.signal'));
    end
end
if strcmp(g('move.continuation.signal'),'stageExhaustion') && ...
        ~strcmp(g('move.policy'),'ladder')
    error('olh:config:exhaustionNeedsLadder', ...
        ['move.continuation.signal=''stageExhaustion'' advances the ladder, so ' ...
