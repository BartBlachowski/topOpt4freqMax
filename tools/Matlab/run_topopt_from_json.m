...
%   diagnostics : solver diagnostics (optional; ourApproach only)
    ...
    if gateA0Diagnostics
        % --- Gate A0 / revision_v1 authoritative-load constraints ---
        if hasSemiHarmonicRhoSource
        ...
        if hasFieldPath(cfg, {'optimization','harmonic_normalize'})
            if parseBool(getFieldPath(cfg, {'optimization','harmonic_normalize'}), ...
                    'optimization.harmonic_normalize')
                error('run_topopt_from_json:GateA0ObsoleteNormalize', ...
        ...
        if hasSemiHarmonicBaseline && ~strcmpi(semiHarmonicBaseline, 'solid')
            error('run_topopt_from_json:GateA0NonSolidBaseline', ...
                ['Gate A0 / revision_v1 configuration requires ' ...
                 'optimization.semi_harmonic_baseline="solid" (got "%s"). ' ...
                 'The authoritative load requires a fully solid reference design.'], ...
                semiHarmonicBaseline);
        end
        if ~hasFieldPath(cfg, {'optimization','load_sensitivity'})
            error('run_topopt_from_json:GateA0MissingLoadSensitivity', ...
...
%   are only meaningful for dynamic (frequency) analyses.
    dynamicOptKeys = {'harmonic_normalize','debug_semi_harmonic', ...
                      'semi_harmonic_baseline','semi_harmonic_rho_source', ...
                      'harmonic_baseline','load_sensitivity','gate_a0_diagnostics','yuksel'};
    optBlock = struct();
