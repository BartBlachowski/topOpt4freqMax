...
%
%   man = OLHOFFCURRENT_SOURCE_MANIFEST() hashes every file under +impl/ and
%   returns the manifest plus a single tree hash that stands for the whole
%   implementation.  The tree hash is the SHA-256 of the sorted
...
function out = local_listFiles(d)
out = {};
L = dir(d);
for k = 1:numel(L)
    if strcmp(L(k).name,'.') || strcmp(L(k).name,'..'); continue; end
    p = fullfile(d, L(k).name);
    if L(k).isdir; out = [out, local_listFiles(p)]; %#ok<AGROW>
    else,          out{end+1} = p; %#ok<AGROW>
    end
end
end
