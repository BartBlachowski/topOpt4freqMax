% run_clamped_clamped.m — Clamped-clamped beam benchmark for TopIQP
%
% Geometry and boundary conditions are ported from the Olhoff-inspired
% frequency-maximization example:
%   analysis/OlhoffApproach/Matlab/run_clamped_clamped.m
...
% 240x30 mesh, both edges fully clamped). TopIQP minimises compliance, so
% there is no load to reuse; a top-edge, mid-span point load fy=-1
% (three-point bending) is added here — see topIQP.m bcType='cc'.
%
...
fprintf('=== Clamped-Clamped 240x30, volfrac=0.5 (method=%s) ===\n', method);
...
figure('Name','Clamped-Clamped 240x30 V=0.5','Color','white');
imagesc(1 - r.xPhys); colormap(gray); axis equal off;
title(sprintf('Clamped-Clamped 240\\times30,  V=0.5,  c=%.4g,  iter=%d', r.compliance, r.nIter));
