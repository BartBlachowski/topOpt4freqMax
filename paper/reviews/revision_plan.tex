...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item If they are spurious modes: note that they arise from using $d=p=3$
        ...
        the local Olhoff-inspired comparison implementation, which uses
        $d=6$ for low-density elements).  This is a comparison of repository
        interpolation choices, not a statement about the published method.
        Reference the Reviewer~3
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item In the paper, add a sentence in Section~4.2 (proposed approach)
        ...
        localized modes in low-density regions (as also noted for the local
        Olhoff-inspired comparison implementation, which uses $d=6$ for this
        reason).  State that this is a local implementation choice and that MAC
...
\]
remains the appropriate way to separate iteration count from average
per-iteration cost.  Any retained performance table must, however, report
measurements for the actual repository implementations and must not imply that
a published Du--Olhoff benchmark was reproduced.

For the Olhoff-related column, the only permitted production source is
\texttt{analysis/OlhoffApproach}.  The column heading and surrounding prose
must call it the \emph{local Olhoff-inspired comparison implementation}.
The table may report its frequency, initialization time, per-iteration time,
iteration count, total time, and memory alongside the other tested local
implementations once the existing A2 timing campaign has been accepted.
These measurements characterize the local code only.
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item convergence-rate comparisons attributed to the published method; and
  \item scaling claims derived from any \texttt{OlhoffApproachExact} output.
\end{itemize}
No replacement experiment is introduced.  Existing performance-planning
items A2 and A6 are retained only to produce and interpret a comparison among
the named local implementations.  Archived reconstruction results are
excluded from every numerical aggregation.
