...
- **2026-09-13 (NOTES §32).** Natural termination without a ladder: preset
  `duOlhoffAdaptiveMove` (per-element box, Svanberg's asyincr/asydecr on the outer
  history; frozen inner loop). At 240×30, r_min = 1.3 el: Fig. 3a topology, M_nd 0.08,
  ω₁ = 171 (−2 %), ω₁ settles by iteration 35–75, printed ε-test fires at 150–340.
  Bimodality holds to ~1 % for 40–100 iterations and is then lost (4–5 %) at a
  localized-mode spike; not closed. 800×100 with the same physical radius and mass
  cut-off 0.2: ω₁ = 168, M_nd 0.10, gap 8 %. Frozen anchors remain bitwise.
  Pedersen (2000) stiffness linearization implemented (`material.stiffness.model =
  'pedersen'`, docs/s001580050130.pdf): removes every localized-mode spike (P1) but the
  pair still opens to 4.6 %; the loss of bimodality is intrinsic, not a spurious-mode effect.
  Nine-mesh sweeps (NOTES §32, `repro/results/SWEEP_R06.md`, `SWEEP_R13EL.md`): preset
  `duOlhoffAdaptivePedersen`. Physical R = 0.06: every mesh terminates naturally, ω₁ 169.7 → 165.8,
  M_nd 0.12–0.17, bimodal only at 160×20. Element r = 1.3: ω₁ reaches 174.7–175.2 at ≥ 560×70,
  M_nd 0.05–0.07, bimodal at 160×20 and 320×40 only, 800×100 hits the cap.
