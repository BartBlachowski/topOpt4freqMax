%% Du--Olhoff 2007 reproduction - Yuksel Figure 4 simply supported problem
clearvars -except optimizer; clc; close all;

thisDir = fileparts(mfilename('fullpath'));
addpath(thisDir);

% Match analysis/YukselApproach/Matlab/run_simply_supported.m.
nelx = 400;
nely = 50;
volfrac = 0.5;
penal = 3;
...
%   optimizer = "mma"; run_olhoff_simply_supported
...
figure('Name','Olhoff reproduced 2007 - simply supported','Color','w');
tiledlayout(2,1,'TileSpacing','compact','Padding','compact');
nexttile;
imagesc(1-reshape(rho,nely,nelx));
set(gca,'YDir','normal'); axis equal tight off; colormap(gray(256)); clim([0 1]);
title(sprintf('Simply supported: \\omega_1 = %.2f rad/s',omega(1)),'Interpreter','tex');
nexttile;
...
fprintf('\nOlhoff reproduced 2007 -- simply supported Yuksel problem\n');
