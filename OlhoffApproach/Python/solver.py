...
class OlhoffConfig:
    ...
    eigs_strategy: str = "shift-invert"
...
def _assemble_global(
    ...
    K = sparse.coo_matrix((sK, (iK, jK)), shape=(n_dof, n_dof)).tocsc()
    M = sparse.coo_matrix((sM, (iK, jK)), shape=(n_dof, n_dof)).tocsc()
...
def _solve_modes(
    ...
    if mode_name in {"sm", "matlab", "matlab_sm"}:
        kwargs["which"] = "SM"
    else:
        kwargs["which"] = "LM"
        kwargs["sigma"] = 0.0
    if ncv is not None:
        kwargs["ncv"] = int(max(k + 2, min(ncv, n - 1)))
    if v0 is not None and v0.size == n:
        kwargs["v0"] = np.asarray(v0, dtype=np.float64)
    eigvals, eigvecs = spla.eigsh(Kf, **kwargs)
...
def _eval_modes(
    ...
    Kf = Kf.tocsc()
    Mf = Mf.tocsc()
    lam, _, _ = _solve_modes(Kf, Mf, num_modes, mode=cfg.eigs_strategy)
...
def _create_live_plot(enabled: bool, window_title: str, figsize: tuple[float, float]) -> Optional[dict]:
    if not enabled:
        return None
    try:
        ...
        if hasattr(fig.canvas, "manager") and fig.canvas.manager is not None:
            try:
                ...
                pass
        return {"plt": plt, "fig": fig, "ax": ax, "artist": None}
    except Exception:
        return None


def _update_live_plot(plot_state: Optional[dict], x_phys: np.ndarray, nely: int, nelx: int, title: str, plot_binary: bool) -> None:
    ...
    if not plt.fignum_exists(fig.number):
        return
    img = x_phys.reshape((nely, nelx), order="F")
    if plot_binary:
        img = (img > 0.5).astype(float)
    artist = plot_state.get("artist")
    if artist is None:
        artist = ax.imshow(1 - img, cmap="gray", vmin=0.0, vmax=1.0, origin="lower", interpolation="nearest")
        ax.set_axis_off()
        plot_state["artist"] = artist
    else:
        artist.set_data(1 - img)
    ax.set_title(title)
    fig.canvas.draw()
...
def run_optimization(
    ...
    live_plot = _create_live_plot(plot_iterations, f"Olhoff {cfg.support_type} iteration", (9, 2.5))
    ...
    for it in range(1, cfg.maxiter + 1):
        ...
        eigs_strategy = str(cfg.eigs_strategy).lower()
        primary_mode = "sm" if eigs_strategy in ("sm", "matlab", "matlab_sm") else "shift"
        secondary_mode = "shift" if primary_mode == "sm" else "sm"
        v0 = prev_V_low[:, 0] if prev_V_low is not None and prev_V_low.shape[0] == Kf.shape[0] else None
        ncv_primary = min(max(2 * cfg.J + 1, 20), Kf.shape[0] - 1)
        ncv_retry = min(max(4 * cfg.J, 20), Kf.shape[0] - 1)

        flag_eigs = 1
        resmax = np.inf
        try:
            lam_sorted, V_low, resvec = _solve_modes(
                Kf, Mf, cfg.J, mode=primary_mode, tol=1e-8, maxiter=400, ncv=ncv_primary, v0=v0
            )
            resmax = float(np.max(resvec))
            flag_eigs = 0
        except Exception:
            flag_eigs = 1
        ...
        if flag_eigs != 0 or resmax > 1e-3:
            reduce_counter = 5
            if verbose:
                print(f"WARN eigsh failed (flag={flag_eigs}, res={resmax:.2e}) at iteration {it}; reducing move for 5 iterations")
            continue
        ...
        _update_live_plot(
            ...
            nely,
            nelx,
            f"It:{it:3d} beta:{int(beta):2d} omega:{omega_cur:9.3f}",
            plot_binary,
        )
