...
interpolation** up to floor values, E2 is Yuksel's piecewise `x^6` mass law, and E3 is
...
are available. E2 and E3 share the same piecewise `x^6` mass law and differ only in stiffness
floor, so the minimum is closer to two-way in evidential terms. This bounds the objection at
