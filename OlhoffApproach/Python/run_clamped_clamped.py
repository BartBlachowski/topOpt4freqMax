...
def main():
    ...
    paper = dict(init=146.1, opt=456.4)

    omega_best, xPhys_best, diag_out = topFreqOptimization_MMA(cfg, opts)

    print(
        "CC case: omega1 initial={:.1f} (paper {:.1f}) | optimized={:.1f} (paper {:.1f})".format(
            diag_out["initial"]["omega"][0], paper["init"], omega_best, paper["opt"]
        )
    )

    fig, ax = plt.subplots(figsize=(10, 2))
    fig.canvas.manager.set_window_title("Olhoff CC topology")
    ax.imshow(
    ...
    ax.axis("off")
    ax.set_title(f"CC: omega1={omega_best:.1f} (paper: {paper['opt']:.1f})")
    fig.tight_layout()
    plt.show()
