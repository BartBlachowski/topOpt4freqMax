function result = topSQP(nelx, nely, volfrac, penal, rmin, opts)
%TOPSQP  Minimum compliance topology optimisation using the full SQP+
%   (IQP + EQP) loop from:
...
%
%   This is topIQP.m (see that file for the IQP-only phase, mesh/BC/filter
%   setup, and the dual-QP reformulation of Section 5.2) plus the EQP
%   second-order refinement phase (Sections 2.3, 4, 6):
%     1. Solve the IQP sub-problem exactly as in topIQP.m (unchanged).
%     2. Estimate the working set of active bound/volume constraints at the
%        IQP solution (Section 7 tolerance eps4).
%     3. Solve the EQP sub-problem restricted to the free (non-active-bound)
%        elements, using the SAME approximate Hessian Ĥ1 = 2*F^T*K^-1*F as
%        the IQP phase (paper: "the same approximate Hessian is used for
%        both the IQP and the EQP phases", Section 7) -- so Ĥ1 is always
%        positive semi-definite and no inertia correction (Section 2.3.1)
%        is needed. Solved via the sparse expanded system (EQP_e, Section 6)
%        so the dense Hessian/K^-1 is never formed explicitly.
%     4. Compute the contraction parameter beta (Section 2.4) and test the
%        combined step d_iq + beta*d_eq against the merit function before
%        falling back to topIQP.m's plain Armijo backtracking + forced-IQP
%        step on d_iq alone (Section 7).
%
%   Implementation note / simplification: Section 7 describes forcing a
%   full IQP step (up to 5 times) when the combined-step test fails, rather
%   than backtracking. This implementation instead falls back to topIQP.m's
%   existing (validated) Armijo backtrack loop first, with the same 5-time
%   forced-full-step counter only once backtracking itself is exhausted --
%   a strict superset of the paper's described fallback, reusing already-
%   validated machinery instead of re-deriving it.
%
%   Problem (P^c):
%     min   u(t)^T K(t) u(t)
...
%   Usage
%   -----
%     result = topSQP(nelx, nely, volfrac, penal, rmin)
%     result = topSQP(nelx, nely, volfrac, penal, rmin, opts)
%
%   Inputs
%   ------
...
%     opts        : optional struct (same fields as topIQP.m)
%       qpFormulation : 'dual' (default, paper), 'primal', or 'auto'
%       practicalStop : stop stalled quadprog tails using eps4-level KKT
%       maxIter       : outer SQP iteration cap
%       bcType        : 'mbb' (default), 'cantilever', 'cc', 'cs', or 'ss'
%                       -- see topIQP.m for the exact definitions (identical
%                       here; 'cc'/'cs'/'ss' reuse the OlhoffApproach
%                       support geometry with an invented three-point-
%                       bending load).
%
%   Output
%   ------
...
%       stepTypeHistory — per-iteration step type: 'C' combined IQP+EQP,
%                         'B' backtracked IQP-only, 'F' forced full IQP,
%                         'S' small/near-converged step
...
eps4     = 1e-4;    % working-set tolerance (Section 7)
eps5     = 1e-6;    % relaxed merit reduction tolerance (Section 7)
...
if ~any(qp_formulation == ["dual", "primal"])
    error('TopSQP:InvalidQPFormulation', ...
...
% 'cc', 'cs', or 'ss') — identical to topIQP.m, see that file for the full
...
stepTypeHistory = repmat('?', max_iter, 1);

fprintf('TopSQP — minimum compliance (IQP + EQP)\n');
fprintf('  mesh %d x %d,  volfrac %.3f,  penal %.1f,  rmin %.2f\n', ...
        nelx, nely, volfrac, penal, rmin);
fprintf('  QP formulation: %s via quadprog\n', char(qp_formulation));
fprintf('  %5s  %14s  %8s  %4s  %6s  %8s  %8s  %10s  %10s  %10s\n', ...
        'Iter', 'Compliance', 'Vol', 'Step', 'alpha', 'beta', '|d_iq|', 'statErr', 'feasErr', 'compErr');
...
for iter = 1:max_iter
    ...
    switch qp_formulation
        case "dual"
            ...
            if qp_flag <= 0
                warning('TopSQP: quadprog returned flag %d at iter %d. Using zero step.', qp_flag, iter);
                d_iq = zeros(nEl, 1);
    ...
    % ---------------------------------------------------------------------
    % 6. Merit function and model reduction (reference is d_iq alone --
    %    paper eq. 13/14: qred_pi is always evaluated at d_iq, even when
    %    the combined step d_iq + beta*d_eq is tested for acceptance).
    ...
    % ---------------------------------------------------------------------
    % 6.5 EQP phase (Sections 2.3, 2.3.1, 4, 6, 7)
    %
    %   Working set: bound i is active if |d_iq_i - u~_i| < eps4 (upper) or
    %   |d_iq_i - l~_i| < eps4 (lower), with u~_i=1-xTilde_i, l~_i=-xTilde_i.
    %   Volume constraint active if |g_val + a'*d_iq| < eps4.
    %
    %   Same approximate Hessian Ĥ1 = 2*Ffree'*Kf^-1*Ffree as the IQP phase
    %   is reused (paper: "the same approximate Hessian is used for both
    %   the IQP and the EQP phases", Section 7) -- so no exact Hessian /
    %   inertia correction (Section 2.3.1) is needed: Ĥ1 is PSD by
    %   construction. Solved via the sparse expanded system (EQP_e,
    %   Section 6, eq. (26)+expansion) restricted to the free elements, so
    %   the dense Hessian and K^-1 are never formed explicitly.
    % ---------------------------------------------------------------------
    d_eq_full = zeros(nEl, 1);
    beta_c    = 0;
    nEqFree   = 0;

    if ~small_iq_step
        upper_active = abs(d_iq - (1 - xTilde)) < eps4;
        lower_active = abs(d_iq - (-xTilde))    < eps4;
        eqFreeIdx = find(~(upper_active | lower_active));
        nEqFree   = numel(eqFreeIdx);
        vol_active = abs(g_val + a' * d_iq) < eps4;

        if nEqFree > 0
            % Hk*d_iq via the same Ĥ1 = 2*Ffree'*Kf^-1*Ffree used above.
            Kinv_Ftu_d = R \ Rinv_d;                 % Kf^-1 * (Ffree*d_iq)
            Hk_d_iq    = 2 * (Ffree' * Kinv_Ftu_d);  % nEl x 1

            Feq    = Ffree(:, eqFreeIdx);            % nFree x nEqFree
            a_eq   = a(eqFreeIdx);                   % nEqFree x 1 (all 1/nEl)
            rhs_eq = -(df_dx(eqFreeIdx) + Hk_d_iq(eqFreeIdx));

            Zdd = sparse(nEqFree, nEqFree);
            if vol_active
                M   = [Zdd,   Feq',              a_eq;
                       Feq,   -0.5*Kf,            sparse(nFree, 1);
                       a_eq', sparse(1, nFree),   0];
                rhs = [rhs_eq; sparse(nFree, 1); 0];
            else
                M   = [Zdd, Feq'; Feq, -0.5*Kf];
                rhs = [rhs_eq; sparse(nFree, 1)];
            end
            ...
            warning(warnState);
            [~, wid] = lastwarn();
            if any(~isfinite(sol)) || any(strcmp(wid, ...
                    {'MATLAB:singularMatrix', 'MATLAB:nearlySingularMatrix', ...
                     'MATLAB:illConditionedMatrix'}))
                d_eq = zeros(nEqFree, 1);
            else
                d_eq = sol(1:nEqFree);
            end
            d_eq_full(eqFreeIdx) = d_eq;

            if any(d_eq)
                % Contraction parameter beta (Section 2.4): largest beta in
                % (0,1] keeping the complementary (currently inactive)
                % bound and volume constraints feasible at d_iq + beta*d_eq.
                ub_gap = (1 - xTilde(eqFreeIdx)) - d_iq(eqFreeIdx);  % >= 0
                lb_gap = d_iq(eqFreeIdx) - (-xTilde(eqFreeIdx));    % >= 0

                beta_bound = inf;
                pos_mask = d_eq > 0;
                neg_mask = d_eq < 0;
                if any(pos_mask)
                    beta_bound = min(beta_bound, min(ub_gap(pos_mask) ./ d_eq(pos_mask)));
                end
                if any(neg_mask)
                    beta_bound = min(beta_bound, min(lb_gap(neg_mask) ./ (-d_eq(neg_mask))));
                end

                beta_vol = inf;
                if ~vol_active
                    a_d_eq = sum(d_eq_full) / nEl;   % a' * d_eq_full
                    vol_slack_iq = bk(1) - a' * d_iq;
                    if a_d_eq > 0
                        beta_vol = vol_slack_iq / a_d_eq;
                    end
                end

                beta_c = max(0, min([1, beta_bound, beta_vol]));
            end
        end
    end

    % ---------------------------------------------------------------------
    % 7. Acceptance/rejection of the step (Section 2.4 + Section 7 eps5
    %    relaxation). Try the combined IQP+EQP direction first; fall back
    %    to topIQP.m's Armijo backtrack + forced-full-IQP-step otherwise.
    % ---------------------------------------------------------------------
    alpha = 1.0;
    accepted = false;
    ...
    if small_iq_step
        ...
        if any(d_eq_full)
            d_comb = d_iq + beta_c * d_eq_full;
            xTilde_comb = max(0, min(1, xTilde + d_comb));
            xPhys_comb  = full(Hnorm * xTilde_comb);

            Ee_comb  = Ev + (E1 - Ev) * xPhys_comb(:).^penal;
            sK_comb  = reshape(KE(:) * Ee_comb', 64*nEl, 1);
            K_comb   = sparse(double(iK), double(jK), sK_comb, nDof, nDof);
            K_comb   = (K_comb + K_comb') / 2;
            Kf_comb  = K_comb(freeDofs, freeDofs);
            uFree_comb = Kf_comb \ F(freeDofs);
            ue_comb  = zeros(nDof, 1); ue_comb(freeDofs) = uFree_comb;
            ue_comb  = ue_comb(edofMat);
            ce_comb  = sum((ue_comb * KE) .* ue_comb, 2);
            f_comb   = Ee_comb' * ce_comb;
            g_comb   = a' * xTilde_comb - volfrac;
            phi_comb = f_comb + pi_pen * max(0, g_comb);

            required_reduction_comb = max(sigma * qred_req - eps5, 0);
            if phi_comb <= phi_old - required_reduction_comb
                combined_accepted = true;
                accepted   = true;
                step_type  = 'C';
                alpha      = 1.0;
                xTilde_new = xTilde_comb;
                xPhys_new  = xPhys_comb;
                f_new      = f_comb;
                g_new      = g_comb;
                ce_new     = ce_comb;
            end
    ...
    if accepted && ~combined_accepted && ~small_iq_step && alpha > 0 && alpha < min_accepted_alpha
        accepted = false;
    end
    ...
    if ~accepted
        if forced_iq_count < max_forced_iq
            ...
            warning('TopSQP: no acceptable step at iter %d. Returning current iterate.', iter);
            alpha      = 0.0;
            xTilde_new = xTilde;
            xPhys_new  = xPhys;
            f_new      = f_val;
    ...
    % ---------------------------------------------------------------------
    % 8. Update design variables and Lagrange multipliers (Section 2.5:
    %    full trust in the IQP multipliers whenever a full step is
    %    accepted -- combined, forced, or small/converged -- and the
    %    alpha-blended update only for an actual Armijo backtrack).
    % ---------------------------------------------------------------------
    xTilde = xTilde_new;
    xPhys  = xPhys_new;

    if small_iq_step || combined_accepted || step_type == 'F'
        lambda = lam_iq;
        xi     = xi_iq;
        eta_m  = eta_iq;
    else
    ...
    % ---------------------------------------------------------------------
    % 9. KKT error (paper Section 2.1, eqs. 1-9)
    ...
    stepTypeHistory(iter) = step_type;

    fprintf('  %5d  %14.6g  %8.4f  %4s  %6.4f  %6.4f  %8.3e  %10.3e  %10.3e  %10.3e\n', ...
            iter, f_new, mean(xTilde), step_type, alpha, beta_c, norm(d_iq), stat_err, feas_err, comp_err);
    ...
    if small_iq_step
        warning(['TopSQP: IQP step below working-set tolerance at iter %d ', ...
...
result.stepTypeHistory = stepTypeHistory(1:iter);
