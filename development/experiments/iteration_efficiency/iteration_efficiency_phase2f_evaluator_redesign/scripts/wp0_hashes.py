...
extra = [
 'examples/Performance/final_campaign/raw/olhoff/s1_160x20.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_240x30.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_320x40.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_400x50.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_480x60.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_560x70.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_640x80.mat',
 'examples/Performance/final_campaign/raw/olhoff/s1_720x90.mat',
 'analysis/iteration_efficiency_phase2d_delta_audit/PHASE2D_DELTA_AUDIT.md',
 'analysis/iteration_efficiency_phase2d_delta_audit/FINDINGS.csv',
 'analysis/iteration_efficiency_phase2d_delta_audit/SHA256SUMS.txt',
 'analysis/iteration_efficiency_phase2d_delta_audit/audit_provenance.json',
 'analysis/iteration_efficiency_phase2a/+ie2a/exact_count_binary.m',
...
# Phase-2E (delta audit) package integrity, verified against its own manifest
p2e = os.path.join(REPO, 'analysis/iteration_efficiency_phase2d_delta_audit')
e_sums = {}
esp = os.path.join(p2e, 'SHA256SUMS.txt')
if os.path.exists(esp):
    for line in open(esp):
        parts = line.strip().split(None, 1)
        if len(parts) == 2:
            e_sums[parts[1].lstrip('*').strip()] = parts[0]
echk = {n: {'declared': h, 'observed': sha(os.path.join(p2e, n)),
            'match': sha(os.path.join(p2e, n)) == h} for n, h in e_sums.items()}
rec['phase2e_sha256sums_selfcheck'] = echk
rec['phase2e_sha256sums_mismatches'] = [k for k, v in echk.items() if not v['match']]
...
print('phase2e SHA256SUMS mismatch:', rec['phase2e_sha256sums_mismatches'] or 'NONE')
